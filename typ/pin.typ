...
#show: website-page.with(title: "A comparison of some structural models of private information arrival", base: "..")

#context {
  if target() == "html" {
    html.elem("h1", attrs: (class: "title"), [A comparison of some structural models of private information arrival])
  } else {
    align(center, text(size: 2em)[A comparison of some structural models of private information arrival])
  }
}
...
Latest version available at #link("https://ssrn.com/abstract=2564369")[SSRN].

== Previous Versions <prev>

- #link("./JFE_4.pdf")[JFE \#4 2018-12-17]
- #link("./JFE_3.pdf")[JFE \#3 2017-11-24]
- #link("./JFE_2.pdf")[JFE \#2 2017-09-07]
- #link("./JFE_1.pdf")[JFE \#1 2017-03-10]
- #link("./JF_2.pdf")[JF \#2 2016-04-29]
- #link("./JF_1.pdf")[JF \#1 2015-05-01]

= Data <data>

#link("https://www.dropbox.com/s/45b42e89gaafg0n/cpie_data.zip?dl=1")[https://www.dropbox.com/s/45b42e89gaafg0n/cpie_data.zip?dl=1]
...
`cpie_daily.csv` sorted by `permno` and `date`.
...
- `cpie_pin`: Corresponds to CPIE#sub[PIN] in the paper (the PIN model)
- `cpie_dy`: Corresponds to CPIE#sub[DY] in the paper (the DY model)
- `cpie_gpin`: Corresponds to CPIE#sub[GPIN] in the paper (the GPIN model)
- `cpie_owr`: Corresponds to CPIE#sub[OWR] (the Odders-White and Ready (2008) model)
- `cpie_mech`: Is the CPIE#sub[Mech], which is a dummy variable defined as: CPIE#sub[Mech,j,t] = 0 if turn#sub[j,t] < turn#sub[j] (mean), and 1 if turn#sub[j,t] ≥ turn#sub[j] (mean)
- `ret_d`, `ret_o`, `y_e`: Correspond to (r#sub[d], r#sub[o], y#sub[e])
- `n_buys` and `n_sells`: Corresponds to B and S
- `turn`: The sum, corresponding to turn

== PIN Model <eo>

`pin_yearly.csv` sorted by `permno` and `year`.
...
`dy_yearly.csv` sorted by `permno` and `year`.
...
`gpin_yearly.csv` sorted by `permno` and `year`.
...
`owr_yearly.csv` sorted by `permno` and `year`.
...
#link("https://www.dropbox.com/s/6xaa3x5zbmvyyq1/pin-est-1319.zip?dl=1")[https://www.dropbox.com/s/6xaa3x5zbmvyyq1/pin-est-1319.zip?dl=1]
...
- Based on #link("https://wrds-web.wharton.upenn.edu/wrds/ds/wrdsapps/intradayind/taqms/index.cfm?navId=524")[WRDS DTAQ Intraday Indicators].
...
#figure(
  image("../pin/xom-gpin-2018.png", width: 80%),
  caption: [Estimates example -- XOM 2018 GPIN model]
)

= Code <code>

Code is now available on its own GitHub #link("https://github.com/edwinhu/pin-code")[page]. A detailed description of the code is also available #link("./pin-code")[here].
